...
func (w *worker) newWorkLoop(recommit time.Duration) {
	...
	for {
		...
		case <-timer.C:
			...
			if w.isRunning() && (w.chainConfig.Yantra == nil || w.chainConfig.Yantra.Period > 0) {
	...
	for {
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil {
				...
				// Special case, if the consensus engine is 0 period yantra(dev mode),
				// submit sealing work here since all empty submission will be rejected
				// by yantra. Of course the advance sealing(empty submission) is disabled.
				if w.chainConfig.Yantra != nil && w.chainConfig.Yantra.Period == 0 {
...
func (w *worker) prepareWork(genParams *generateParams) (*environment, error) {
	...
	// since yantra algorithm can modify the coinbase field in header.
