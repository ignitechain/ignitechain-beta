...
// checkpoint creates a Yantra checkpoint signer section from the provided list
...
// sign calculates a Yantra digital signature for the given block and embeds it
...
// the account may or may not have cast a Yantra vote.
...
// Tests that Yantra signer voting is evaluated correctly for various simple and
// complex scenarios, as well as that a few special corner cases fail correctly.
func TestYantra(t *testing.T) {
	...
	for i, tt := range tests {
		...
		config.Yantra = &params.YantraConfig{
			Period: 1,
			Epoch:  tt.epoch,
		}
		engine := New(&config, config.Yantra, db, &ethapi.PublicBlockChainAPI{})
		...
		// Pass all the headers through yantra and ensure tallying succeeds
