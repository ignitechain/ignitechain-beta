...
func InspectDatabase(db ethdb.Database, keyPrefix, keyStart []byte) error {
	...
	for it.Next() {
		...
		case bytes.HasPrefix(key, []byte("yantra-")) && len(key) == 7+common.HashLength:
			yantraSnaps.Add(size)
	...
	stats := [][]string{
		...
		{"Key-Value store", "Yantra snapshots", yantraSnaps.Size(), yantraSnaps.Count()},
