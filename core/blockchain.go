...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	// the head block (ethash cache or yantra voting snapshot). Might as well do
	// it in advance.
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals, setHead bool) (int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		// Yantra blocks where they can share state among each other, so importing an
		...
		if bc.skipBlock(err, it) {
			...
			// block in the middle. It can only happen in the yantra chain. Whenever
			...
			// Yantra blocks to have the same state is if there are no transactions.
			lastCanon = block
			continue
		}
