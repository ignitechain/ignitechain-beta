...
func (g *Genesis) Commit(db ethdb.Database) (*types.Block, error) {
	...
	if config.Yantra != nil && len(block.Extra()) < 32+crypto.SignatureLength {
		return nil, errors.New("can't start yantra chain without signers")
	}
...
func DeveloperGenesisBlock(period uint64, gasLimit uint64, faucet common.Address) *Genesis {
	...
	config := *params.AllYantraProtocolChanges
	config.Yantra = &params.YantraConfig{
		Period: period,
		Epoch:  config.Yantra.Epoch,
	}
