...
//
// Note if we are running the yantra, fetches the last epoch snapshot header
...
func (lc *LightChain) SyncCheckpoint(ctx context.Context, checkpoint *params.TrustedCheckpoint) bool {
	...
	if yantra := lc.hc.Config().Yantra; yantra != nil {
		latest -= latest % yantra.Epoch // epoch snapshot for yantra
	}
	if head >= latest {
		return true
	}
