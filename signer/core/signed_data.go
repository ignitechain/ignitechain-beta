...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	...
	case apitypes.ApplicationYantra.Mime:
		// Yantra is the Ethereum PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useEthereumV, fmt.Errorf("input for %v must be an hex-encoded string", apitypes.ApplicationYantra.Mime)
		}
		yantraData, err := hexutil.Decode(stringData)
		if err != nil {
			return nil, useEthereumV, err
		}
		header := &types.Header{}
		if err := rlp.DecodeBytes(yantraData, header); err != nil {
			return nil, useEthereumV, err
		}
		// The incoming yantra header is already truncated, sent to us with a extradata already shortened
		...
		sighash, yantraRlp, err := yantraHeaderHashAndRlp(header)
		if err != nil {
			return nil, useEthereumV, err
		}
		messages := []*apitypes.NameValueType{
			{
				Name:  "Yantra header",
				Typ:   "yantra",
				Value: fmt.Sprintf("yantra header %d [0x%x]", header.Number, header.Hash()),
			},
		}
		// Yantra uses V on the form 0 or 1
		useEthereumV = false
		req = &SignDataRequest{ContentType: mediaType, Rawdata: yantraRlp, Messages: messages, Hash: sighash}
...
// yantraHeaderHashAndRlp returns the hash which is used as input for the proof-of-authority
// signing. It is the hash of the entire header apart from the 65 byte signature
// contained at the end of the extra data.
//
// The method requires the extra data to be at least 65 bytes -- the original implementation
// in yantra.go panics if this is the case, thus it's been reimplemented here to avoid the panic
// and simply return an error instead
func yantraHeaderHashAndRlp(header *types.Header) (hash, rlp []byte, err error) {
	if len(header.Extra) < 65 {
		err = fmt.Errorf("yantra header extradata too short, %d < 65", len(header.Extra))
		return
	}
	rlp = yantra.YantraRLP(header)
	hash = yantra.SealHash(header).Bytes()
	return hash, rlp, err
}
