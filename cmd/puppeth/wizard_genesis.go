...
func (w *wizard) makeGenesis() {
	...
	fmt.Println()
	fmt.Println("Which consensus engine to use? (default = yantra)")
	fmt.Println(" 1. Ethash - proof-of-work")
	fmt.Println(" 2. yantra - ignite protocol")
	...
	case choice == "" || choice == "2":
		// In the case of yantra, configure the consensus parameters
		genesis.Difficulty = big.NewInt(1)
		genesis.Config.Yantra = &params.YantraConfig{
			Period: 15,
			Epoch:  30000,
		}
		fmt.Println()
		fmt.Println("How many seconds should blocks take? (default = 15)")
		genesis.Config.Yantra.Period = uint64(w.readDefaultInt(15))
