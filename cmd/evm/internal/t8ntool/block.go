...
import (
	...
	"github.com/ethereum/go-ethereum/consensus/yantra"
...
type bbInput struct {
	...
	Yantra    *yantraInput `json:"yantra,omitempty"`
...
func (c *yantraInput) UnmarshalJSON(input []byte) error {
	var x struct {
	...
	if x.Key == nil {
		return errors.New("missing required field 'secretKey' for yantraInput")
	}
...
// sealYantra seals the given block using yantra.
func (i *bbInput) sealYantra(block *types.Block) (*types.Block, error) {
	// If any yantra value overwrites an explicit header value, fail
	// to avoid silently building a block with unexpected values.
	if i.Header.Extra != nil {
		return nil, NewError(ErrorConfig, fmt.Errorf("sealing with yantra will overwrite provided extra data"))
	}
	header := block.Header()
	if i.Yantra.Voted != nil {
		if i.Header.Coinbase != nil {
			return nil, NewError(ErrorConfig, fmt.Errorf("sealing with yantra and voting will overwrite provided coinbase"))
		}
		header.Coinbase = *i.Yantra.Voted
	}
	if i.Yantra.Authorize != nil {
		if i.Header.Nonce != nil {
			return nil, NewError(ErrorConfig, fmt.Errorf("sealing with yantra and voting will overwrite provided nonce"))
		}
		if *i.Yantra.Authorize {
			header.Nonce = [8]byte{}
		} else {
	...
	copy(header.Extra[0:32], i.Yantra.Vanity.Bytes()[:])

	// Sign the seal hash and fill in the rest of the extra data
	h := yantra.SealHash(header)
	sighash, err := crypto.Sign(h[:], i.Yantra.Key)
	if err != nil {
		return nil, err
	}
...
func readInput(ctx *cli.Context) (*bbInput, error) {
	var (
		...
		yantraStr  = ctx.String(SealYantraFlag.Name)
	...
	if ethashOn && yantraStr != "" {
		return nil, NewError(ErrorConfig, fmt.Errorf("both ethash and yantra sealing specified, only one may be chosen"))
	}
	if ethashOn {
	...
	if headerStr == stdinSelector || ommersStr == stdinSelector || txsStr == stdinSelector || yantraStr == stdinSelector {
		decoder := json.NewDecoder(os.Stdin)
		if err := decoder.Decode(inputData); err != nil {
			return nil, NewError(ErrorJson, fmt.Errorf("failed unmarshaling stdin: %v", err))
		}
	}
	if yantraStr != stdinSelector && yantraStr != "" {
		var yantra yantraInput
		if err := readFile(yantraStr, "yantra", &yantra); err != nil {
			return nil, err
		}
		inputData.Yantra = &yantra
	}
