...
func testExternalUI(api *core.SignerAPI) {
	...
	{ // Sign data test - yantra header
		api.UI.ShowInfo("Please approve the next request for signing a yantra header")
		time.Sleep(delay)
		yantraHeader := types.Header{
		...
		yantraRlp, err := rlp.EncodeToBytes(yantraHeader)
		if err != nil {
			utils.Fatalf("Should not error: %v", err)
		}
		addr, _ := common.NewMixedcaseAddressFromString("0x0011223344556677889900112233445566778899")
		_, err = api.SignData(ctx, accounts.MimetypeYantra, *addr, hexutil.Encode(yantraRlp))
		expectApprove("signdata - yantra header", err)
	}
