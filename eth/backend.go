...
func (s *Ethereum) shouldPreserve(header *types.Header) bool {
	// The reason we need to disable the self-reorg preserving for yantra
	...
	// network is stuck.
	if _, ok := s.engine.(*yantra.Yantra); ok {
		return false
	}
...
func (s *Ethereum) StartMining(threads int) error {
	...
	if !s.IsMining() {
		...
		var cli *yantra.Yantra
		if c, ok := s.engine.(*yantra.Yantra); ok {
			cli = c
		} else if cl, ok := s.engine.(*beacon.Beacon); ok {
			if c, ok := cl.InnerEngine().(*yantra.Yantra); ok {
				cli = c
			}
		}
		if cli != nil {
